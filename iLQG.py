...
class nonlin_sys():
    ...
    def dyn(self, x,u):
        ...
        # x = [position, velocity]
        # u = [force]
        m, b, k, dt = 0.3, 10, 100, .05 
        #v = x[1]+dt/m*(u+k*m.max(x[0],0))
        #x_next = x+np.array([dt*x[1], dt/m*(u[0]-b*x[1]-k*x[0])], dtype=object)
        x_next = x+np.array([dt*x[1], dt/m*(u[0]-b*x[1])], dtype=object)
        return x_next 

    
    def cost_stage(self, x,u):
        m = sym if x.dtype == object else np # check for autodiff
        c = x[0]**2 + 0.1*x[1]**2 + 0.1*u**2
        return c 
    ...
    def rollout(self, x0, u_trj):
        ...
        for i in range(u_trj.shape[0]):
            x_trj[i+1,:] = self.dyn(x_trj[i,:],u_trj[i])    
        return x_trj
...
class iLQR():
    ...
    def forward_pass(x_trj, u_trj, k_trj, K_trj, sys):
        ...
        for n in range(u_trj.shape[0]):
            ...
            x_trj_new[n+1,:] = sys.dyn(x_trj_new[n,:],u_trj_new[n,:])
...
def run_ilqr(x0, N, sys, max_iter=50, regu_init=100):
    # First forward rollout
    u_trj = np.random.randn(N-1, sys.n_u)*0.0001
    x_trj = sys.rollout(x0, u_trj)
    ...
    for it in range(max_iter):
        ...
        x_trj_new, u_trj_new = iLQR.forward_pass(x_trj, u_trj, k_trj, K_trj, sys)
...
x_trj, u_trj, cost_trace, regu_trace, redu_ratio_trace, redu_trace = run_ilqr(x0, N, test_sys, max_iter, regu_init)
